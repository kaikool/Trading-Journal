...
interface ImageUploadProps {
  ...
  onCaptureClick?: () => void;   // trigger capture API (re-capture thủ công nếu cần)
  onRemoveImage: () => void;
  inputId?: string;              // giữ để không vỡ props cũ (không dùng)
}
...
export function ImageUpload({
  ...
  return (
    <div className="overflow-visible">
      {preview ? (
        <div className="relative rounded-lg overflow-hidden border border-border/40 group shadow-sm">
          ...
          <div className="absolute top-0 left-0 right-0 p-2 z-10 bg-gradient-to-b from-black/50 to-transparent">
            <div className="flex items-center justify-between">
              ...
              {uploadSuccess && (
                <div className="bg-green-500/80 text-white rounded-full p-0.5 w-4 h-4 flex items-center justify-center backdrop-blur-sm" aria-label="Captured successfully">
          ...
          {/* Bottom progress bar while capturing */}
          {isUploading && (
        ...
        <div className="rounded-lg bg-muted/10 border border-dashed border-border overflow-hidden hover:bg-muted/20 transition-colors p-4">
          <div className="w-full flex flex-col items-center justify-center py-4">
            ...
            {/* Nút chỉ là fallback — auto-capture đã được kích từ TradeImages.tsx */}
            {onCaptureClick && (
              <Button
                type="button"
                size="sm"
      ...
      {error && (
        <div className="text-xs text-destructive flex items-center gap-1 mt-1.5 px-1.5 rounded-md" role="alert">
          <span>{error}</span>
        </div>
      )}
    </div>
